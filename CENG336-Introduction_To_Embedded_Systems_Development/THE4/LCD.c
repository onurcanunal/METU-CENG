...
void Lcd_refresh(void)
{
  ...
  for (i = 0; i < LINE; i++)
  {
    Lcd_position(i, 0);
    for (j = 0; j < COLUMN; j++){
        if(lcdToggle == 0) WriteDataToLCD(map[i][j]);       // When lcdToggle is 0 display the first 2 rows of the map to LCD
        else               WriteDataToLCD(map[i+2][j]);     // When lcdToggle is 1 display the last 2 rows of the map to LCD
    }
  }
}
