...
void keyboardCallback(GLFWwindow* win, int key, int scancode, int action, int mods)
{
    if(key == GLFW_KEY_R && (action == GLFW_PRESS || action == GLFW_REPEAT)){ //rise
      ...
      counter = (counter - 1 + widthTexture) % widthTexture;
    }
    else if(key == GLFW_KEY_E && (action == GLFW_PRESS || action == GLFW_REPEAT)){ //right
      counter = (counter + 1) % widthTexture;
    }
    ...
    ////////////////////????????????????????*******************************
      ...
      // light configuration is not stated in the homework text
      v.x = 0.0; v.y = 1.0; v.z = 0.0;
      gaze.x = 0.0; gaze.y = 0.0; gaze.z = 1.0;
      u.x = 1.0; u.y = 0.0; u.z = 0.0;
      e.x = widthTexture/2.0; e.y = widthTexture/10.0; e.z = widthTexture/-4.0;
      cameraSpeed = 0;     
      counter = 0; 
    }
...
void updateCameraSettings(){
  ...
  glm::mat4 perspectiveProjectionMatrix = glm::perspective(45.0f, 1.0f, 0.1f, 1000.0f);
...
int main(int argc, char * argv[]) {
  ...
  e = glm::vec3(widthTexture/2.0,widthTexture/10.0,widthTexture/-4.0);
  updateCameraSettings();
  lightPosition = glm::vec3(widthTexture/2.0, 100.0, heightTexture/2.0);  
  updateLightSetting();
  ...
  numberOfVertices = 3 * 2 * widthTexture * heightTexture;
  ...
  for(i=0; i<widthTexture; i++){
    for(j=0; j<heightTexture; j++){
      vertices[vertexIndex++] = glm::vec3(i, 0, j);
      vertices[vertexIndex++] = glm::vec3(i+1, 0, j+1);
      vertices[vertexIndex++] = glm::vec3(i+1, 0, j);
      vertices[vertexIndex++] = glm::vec3(i, 0, j+1);
      vertices[vertexIndex++] = glm::vec3(i, 0, j);
      vertices[vertexIndex++] = glm::vec3(i+1, 0, j+1);
    }
  }
