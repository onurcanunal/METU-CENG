...
void main() {
  ...
  vec4 diffuse = kd * max(0, dot(normalize(ToLightVector), vertexNormal)) * Id;
  // compute specular component
  vec4 specular = ks * pow(max(0, dot(vertexNormal, normalize(normalize(ToLightVector) + normalize(ToCameraVector)))), specExp) * Is;
  ...
  //color = vec4(normalize(ToCameraVector),1);
}
